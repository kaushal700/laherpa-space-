...
const ServerAfrica = () => {
  useEffect(() => {
    document.title = "Africa Server - La Herpaile";
  }, []);
  ...
  const locations = [
    {
      ...
      region: "South Africa",
      latency: "< 2ms",
      features: ["African Hub", "Tech Innovation", "Premium Tier-1"],
      flag: "🇿🇦"
    },
  ];

  const features = [
    { icon: Zap, title: "African Speed", description: "Optimized connectivity across the entire African continent from Cape Town" },
    { icon: Shield, title: "Local Data Center", description: "Secure infrastructure within Africa with POPIA compliance" },
    { icon: Clock, title: "99.99% Uptime", description: "Reliable service with regional redundancy and backup power" },
    { icon: Users, title: "Pan-African Support", description: "Multilingual support across all African time zones 24/7" },
  ];

  const specs = [
    { icon: MapPin, text: "Cape Town Data Center" },
    { icon: Cpu, text: "Intel Xeon Processors" },
  ...
  const coverage = [
    { country: "Nigeria", flag: "🇳🇬", latency: "< 50ms" },
    { country: "Kenya", flag: "🇰🇪", latency: "< 45ms" },
    { country: "Egypt", flag: "🇪🇬", latency: "< 60ms" },
    { country: "Morocco", flag: "🇲🇦", latency: "< 65ms" },
    { country: "Ghana", flag: "🇬🇭", latency: "< 55ms" },
    { country: "Tanzania", flag: "🇹🇿", latency: "< 48ms" },
  ];
    ...
    <main className="min-h-screen relative overflow-hidden">
      ...
      <section className="relative min-h-[80vh] flex items-center justify-center overflow-hidden pt-32">
        <div className="absolute inset-0">
          <img src={serverHero} alt="Africa Server" className="w-full h-full object-cover" />
        ...
        <div className="container relative z-10 text-center px-4">
          <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-accent/30 to-secondary/30 backdrop-blur-md rounded-full border border-accent/40 mb-8 shadow-lg shadow-accent/20">
            <span className="text-accent font-bold text-lg">🌍 Africa</span>
          </div>

          <h1 className="text-6xl md:text-8xl font-display font-bold text-white mb-8">
            Africa <span className="text-gradient-warm drop-shadow-lg">Premium Server</span>
          </h1>
          <p className="text-xl md:text-2xl text-white/90 max-w-3xl mx-auto mb-10 leading-relaxed">
            Empowering African innovation from <span className="text-accent font-semibold">Cape Town, South Africa</span> with local high-performance hosting infrastructure
          </p>
      ...
      <section className="py-24 bg-gradient-to-b from-background via-card/50 to-background relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <span className="inline-block px-6 py-3 bg-gradient-to-r from-accent/25 to-secondary/25 rounded-full text-accent font-bold mb-6 text-lg shadow-lg">
              🗺️ Primary Location
            </span>
            <h2 className="text-4xl md:text-6xl font-display font-bold mb-6">
              <span className="text-gradient-warm">Cape Town</span> Data Center
            </h2>
            <p className="text-muted-foreground text-xl max-w-2xl mx-auto">
              The gateway to Africa's digital future
            </p>
          </div>
          ...
          <div className="max-w-2xl mx-auto mb-16">
            {locations.map((location, index) => (
              <Link
                key={index}
                to="/server-info"
                className="group relative block p-12 rounded-3xl bg-gradient-to-br from-card via-card/80 to-background border border-border/50 hover:border-accent/50 transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl hover:shadow-accent/20 overflow-hidden"
              >
                ...
                <div className="relative z-10 text-center">
                  ...
                  <h3 className="text-4xl font-bold mb-2 group-hover:text-accent transition-colors">{location.city}</h3>
                  <p className="text-muted-foreground text-xl mb-4">{location.region}</p>
                  <span className="inline-block px-6 py-3 bg-accent/20 text-accent text-xl font-bold rounded-full mb-6">
          ...
          <div className="text-center mb-12">
            <h3 className="text-2xl md:text-3xl font-bold mb-4">Pan-African Coverage from Cape Town</h3>
            <p className="text-muted-foreground">Connectivity to all major African nations</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {coverage.map((item, index) => (
              <div
                key={index}
                className="p-4 rounded-2xl bg-card/50 border border-border/30 text-center hover:border-accent/30 transition-all duration-300 hover:shadow-lg group"
              >
                <span className="text-3xl block mb-2">{item.flag}</span>
                <h4 className="font-semibold group-hover:text-accent transition-colors">{item.country}</h4>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <span className="inline-block px-6 py-3 bg-gradient-to-r from-accent/25 to-secondary/25 rounded-full text-accent font-bold mb-6 text-lg">
              ⚡ Why Choose Africa Server
            </span>
            <h2 className="text-4xl md:text-6xl font-display font-bold mb-6">
              <span className="text-gradient-warm">Pan-African</span> Excellence
            </h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              ...
              >
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-accent/25 to-secondary/25 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg">
